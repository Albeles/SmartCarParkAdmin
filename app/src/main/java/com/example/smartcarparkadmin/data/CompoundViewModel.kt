...
class CompoundViewModel : ViewModel(){
    ...
    private val col = Firebase.firestore.collection("compound")
    ...
    suspend fun addCompound( carplate: String):Boolean {
    ...
    suspend fun addCC(ctx: Context,carplate: String): Boolean {
        ...
        db.collection("users").document(users.id).update("name",name,"compoundCount",ccount.toInt())
        return true
    }
